...
def s3_file_exists(client, bucket_name, object_name):
...
def import_from_s3(course_id, timestamp):
    ...
    while not s3_file_exists(client, settings.MODULE_EXPORTIMPORT_S3_BUCKET, object_name) and datetime.now() < retry_until:
        time.sleep(3)
