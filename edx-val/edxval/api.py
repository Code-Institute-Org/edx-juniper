...
def _get_video(edx_video_id):
    ...
    try:
        ...
        error_message = "Video not found for edx_video_id: {}".format(edx_video_id)
        raise ValVideoNotFoundError(error_message) from no_video_error
    except Exception as exc:
        error_message = "Could not get edx_video_id: {}".format(edx_video_id)
...
def get_video_info_for_course_and_profiles(course_id, profiles):
    ...
    try:
        ...
        error_message = "Could not get encoded videos for course: {}".format(course_id)
        logger.exception(error_message)
        raise ValInternalError(error_message) from exc
